...
def draw_trim_marks(
    ...
    """
    Draw trim marks at the sheet edges marking cut lines between strips.

    For standard layout (mini_rows=1): marks at top/bottom for vertical cuts only.
    For miniature layout (mini_rows=2): also draws marks at left/right for the
    horizontal mid-page cut.
    """
    strip_w = page_w / strips_per_page
    c.saveState()
    c.setLineWidth(0.5)

    # Vertical cut marks (top and bottom edges)
    for i in range(1, strips_per_page):
        x = i * strip_w
        c.line(x, 0, x, mark_len)
        c.line(x, page_h - mark_len, x, page_h)

    # Horizontal cut marks (left and right edges) — miniature only
    if mini_rows > 1:
        strip_h_row = page_h / mini_rows
        for j in range(1, mini_rows):
            y = j * strip_h_row
            c.line(0, y, mark_len, y)
            c.line(page_w - mark_len, y, page_w, y)
...
def make_pdf(
    ...
    if miniature:
        # 5 columns × 2 rows = 10 strips per sheet
        _cols = 5
        _rows = 2
        strip_w = page_w / _cols
        strip_h = page_h / _rows
    else:
        _cols = strips_per_page
        _rows = 1
        strip_w = page_w / strips_per_page
    ...
    # For standard layout, center the label area vertically on the sheet.
    # For miniature, strips fill the full page so no centering offset is needed.
    _std_y0 = (page_h - strip_h) / 2.0
    ...
    strips_per_sheet = _cols * _rows

    idx = 0
    while idx < len(rows):
        if show_trim_marks:
            draw_trim_marks(c, page_w, page_h, _cols, mini_rows=_rows)

        # page_strips: list of (col, y_strip_bottom, row) for this sheet
        page_strips: List[tuple] = []

        for strip_idx in range(strips_per_sheet):
            if idx >= len(rows):
                break

            col = strip_idx % _cols
            visual_row = strip_idx // _cols  # 0 = top, 1 = bottom
            if miniature:
                # visual_row 0 (top) → higher y value in ReportLab coords
                y_start = (_rows - 1 - visual_row) * strip_h
            else:
                y_start = _std_y0

            row = rows[idx]
            idx += 1
            page_strips.append((col, y_start, row))
        ...
        # Verso page: barcodes for short-edge duplex printing
        if with_barcodes and any(r.barcode for _, _, r in page_strips):
            if miniature:
                # Miniature verso: short-edge (top-to-bottom) flip swaps the two rows;
                # columns stay in the same left-right order.
                if show_trim_marks:
                    draw_trim_marks(c, page_w, page_h, _cols, mini_rows=_rows)
                ...
                for _ in range(20):
                    fits = True
                    for _, _, r in page_strips:
                        if not r.barcode:
                            continue
                        probe = Code128(r.barcode, barWidth=bar_width,
                                        barHeight=bc_height, humanReadable=False)
                        if probe.width > max_bc_width:
                            fits = False
                            break
                    if fits:
                        break
                    bar_width = round(bar_width - 0.05, 3)
                bar_width = max(bar_width, 0.5)

                for col, strip_y0, row in page_strips:
                    if not row.barcode:
                        continue
                    # Swap rows: top-row strips land in bottom position after flip, vice versa
                    verso_y0 = (_rows - 1) * strip_h - strip_y0
                    bc = Code128(row.barcode, barWidth=bar_width, barHeight=bc_height,
                                 humanReadable=True, fontSize=5)
                    x_strip = col * strip_w
                    bc_x = x_strip + (strip_w - bc.width) / 2
                    bc_y = verso_y0 + 0.198 * inch
                    c.saveState()
                    c.translate(bc_x + bc.width / 2, bc_y + bc.height / 2)
                    c.rotate(180)
                    bc.drawOn(c, -bc.width / 2, -bc.height / 2)
                ...
                page_rows = [r for _, _, r in page_strips]
